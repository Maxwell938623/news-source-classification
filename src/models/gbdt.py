...
_to_dense = FunctionTransformer(
    lambda X: X.toarray() if sp.issparse(X) else np.asarray(X, dtype=np.float64),
    accept_sparse=True,
)
...
def _gbdt_pipeline(
    ...
    4. clf           → HistGradientBoostingClassifier
    """
    if clf is None:
